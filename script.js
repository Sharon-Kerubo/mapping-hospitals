...
var storeLocationDataUrl = 'data/Health_Facilities_in_Kenya.txt';

//The URL to the icon image. 
var iconImageUrl = 'images/hospital.png';
            ...
            for (i = 1; i < lines.length; i++) {
                ...
                if (row.length >= numColumns) {
                    ...
                    features.push(new atlas.data.Feature(new atlas.data.Point([parseFloat(row[header['Longitude']]), parseFloat(row[header['Latitude']])]), {
                        ...
                        Y: row[header['Y']],
                        FID: row[header['FID']],
                        Facility_N: row[header['Facility_N']],
                        F_NAME: row[header['F_NAME']],
                        HMIS: row[header['HMIS']],
                        Province: row[header['Province']],
                        District: row[header['District']],
                        Division: row[header['Division']],
                        LOCATION: row[header['LOCATION']],
                        Sub_Locati: row[header['Sub_Locati']],
                        Spatial_Re: row[header['Spatial_Re']],
                        Facility_T: row[header['Facility_T']],
                        Agency: row[header['Agency']],
                        GlobalID: row[header['GlobalID']],
                    }));
                }
            }
...
function getAddressLine2(properties) {
    ...
    if (properties['Division']) {
     ...
     if (properties['Sub_Locati']) {
        html.push(' ', properties['Sub_Locati']);
    }
